...
|-- cli/           # Rust zamapay command-line helper for merchant integration checks
...
- `cli` depends on `shared` and `webhook-verifier` so command behavior follows the same merchant API and webhook contracts as the platform.
