...
static SCHEMA_SQL: &[&str] = &[
    ...
    create table if not exists zamapay_evm_chain_tokens (
        ...
        supports_eip3009 boolean not null default false,
        supports_permit2 boolean not null default false,
        supports_erc2612_permit boolean not null default false,
        requires_standard_approve boolean not null default true,
        permit2_contract text,
        eip712_domain_name text,
        eip712_domain_version text,
        enabled boolean not null,
        primary key (state_key, token_id)
    )
    "#,
    "alter table zamapay_evm_chain_tokens add column if not exists supports_eip3009 boolean not null default false",
    "alter table zamapay_evm_chain_tokens add column if not exists supports_permit2 boolean not null default false",
    "alter table zamapay_evm_chain_tokens add column if not exists supports_erc2612_permit boolean not null default false",
    "alter table zamapay_evm_chain_tokens add column if not exists requires_standard_approve boolean not null default true",
    "alter table zamapay_evm_chain_tokens add column if not exists permit2_contract text",
    "alter table zamapay_evm_chain_tokens add column if not exists eip712_domain_name text",
    "alter table zamapay_evm_chain_tokens add column if not exists eip712_domain_version text",
