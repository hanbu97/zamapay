...
```text
contracts/scripts
|-- deploy-contracts.js     # Deploy local-dev contracts and write manifest/billing/ERC20/Permit2 terms
...
- `deploy-contracts.js` deploys `PrivateCheckoutSettlement`, `EvmCheckoutSettlement`, `ConfidentialUSDMock`, subscription contracts, local transparent USDT/USDC mocks, and the local Permit2-compatible signature-transfer contract; `ZAMAPAY_EVM_WITHDRAW_AUTHORIZER` and `ZAMAPAY_PLATFORM_FEE_WALLET` define the ordinary ERC20 custody boundary for non-local deployments.
