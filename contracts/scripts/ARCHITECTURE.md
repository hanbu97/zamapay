...
```text
contracts/scripts
|-- deploy-contracts.js             # Deploy current Hardhat network contracts and write address + billing-term manifest
...
- `deploy-contracts.js` reads deployed subscription/settlement constants and writes billing terms through the same bridge, so web and Rust never need to duplicate contract fee policy.
...
- `mint-test-usd.js` is the explicit buyer-liquidity step for demos: the contract owner mints into the buyer's confidential balance; local browser QA now displays that balance in the app instead of MetaMask's ERC20 token list.
