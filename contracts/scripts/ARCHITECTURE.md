...
```text
contracts/scripts
|-- deploy-contracts.js             # Deploy current Hardhat network contracts and write address manifest
...
|-- project-finalized-payment.js    # Parse InvoicePaid tx and project payment/finality into Rust
...
|-- sync-generated.js               # Copy ABI truth into generated/* clients and manifests
`-- verify-sepolia-readiness.js     # Sepolia RPC, deployer, buyer, secrets, web env, manifest, bytecode, and API readiness gate
```

## Decisions

- `sync-generated.js` is the canonical bridge from Hardhat artifacts into repo-level generated clients.
- `sync-generated.js` preserves the current address manifest when only ABI/client regeneration is requested.
- `deploy-contracts.js` writes addresses through the same bridge, so web and Rust never need to read Hardhat artifact directories directly.
...
- `mint-test-usd.js` is the explicit buyer-liquidity step for demos: the contract owner mints clear test mcUSD into the buyer's confidential balance before browser payment.
...
- `project-finalized-payment.js` is the manual operator bridge for demos without a persistent indexer: it parses an `InvoicePaid` transaction, posts payment projection, then advances confirmations.
- `smoke-local-invoice.js` proves wallet-owned invoice creation, confidential token mint/approval, exact encrypted buyer payment, public payment-check decryption, settlement finalization, chain-id projection, confirmation advancement, and demo card-code release.
