...
- `lib/cardforge-api.ts` calls only the CardForge backend checkout, fulfillment, and webhook-log endpoints without browser credentials, so Mermer Pay session cookies never enter the demo backend.
