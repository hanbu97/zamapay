...
- `ProductCoverflow.tsx` warms product-scoped checkout invoices on load, hover, and focus, then sends selected product id and optional connected buyer wallet only to the merchant backend. It never sends merchant wallet, payout wallet, raw order id, or client-chosen amount.
