...
- Ordinary EVM checkout uses the platform-created payment intent: exact token amount, settlement contract, settlement intent id, project id, fee split, network, and expiry come from Rust; the browser only approves the token and calls `EvmCheckoutSettlement.pay`.
