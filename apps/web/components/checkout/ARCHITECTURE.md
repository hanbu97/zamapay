...
`-- CheckoutPaymentCard.tsx         # Buyer wallet payment client: local-dev private intent
```

## Decisions

- Checkout client code owns only browser-bound actions: wallet chain switching, buyer intent signature for local-dev, and backend projection after verified finalization.
...
- Local Hardhat hosted checkout signs a private payment intent; the dev-gated relayer route submits `PrivateCheckoutSettlement`, debits `MockConfidentialPaymentRail`, decrypts only `accepted`, and never asks MetaMask to show or transfer cUSDT.
