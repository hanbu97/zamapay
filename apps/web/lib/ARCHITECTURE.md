...
|-- local-fhevm-dev.ts      # Server-only Hardhat FHEVM mock bridge for checkout creation and finalization
...
- `local-fhevm-dev.ts` is server-only and gated by API routes; it creates `PrivateCheckoutSettlement` checkouts for merchant backends and finalizes submitted checkout payments so buyers confirm only one wallet transaction.
